...
class SortedTableMap(MapBase):
    ...
    def _find_index(self, k, low, high):
        ...
        if high < low:
            ...
            if k == self._table[mid]._key:
                ...
                return self._find_index(k, mid + 1, high)  # answer, out of mid
    ...
    def find_gt(self, k):
        ...
        def find_lt(self, k):
            """Return (key, value) pair with greatest key strictly
            less than k."""
            j = self._find_index(k, 0, len(self._table) - 1)
            if j < len(self._table):
                return (self._table[j-1]._key, self._tablep[j-1]._value)
            else:
                return None

    def find_le(self, k):
        """Return (key, value) pair with key less than or equal to k."""
        j = self._find_index(k, 0, len(self._table) - 1)
        if self._table[j]._key == k:
            return (self._table[j]._key, self._table[j]._value)
        elif j > 0:
            return (self._table[j-1]._key, self.table[j-1]._value)
        else:
            return None
